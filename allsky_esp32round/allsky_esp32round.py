...
class ALLSKYESP32ROUND():
    ...
    def create_image(self, roi, size, quality, filename):
        if quality is not None:
            quality = int(quality)
            binning = allsky_shared.getEnvironmentVariable('AS_BIN')
            ...
            save_dir = allsky_shared.getEnvironmentVariable("CAPTURE_SAVE_DIR")
            if save_dir is None:
                save_dir = allsky_shared.getEnvironmentVariable("ALLSKY_CURRENT_DIR")
    ...
    def run(self):
        ...
        enabled34 = self.params['enable34']
        enabled4 = self.params['enable4']
        size34 = int(self.params['size34'])
        size4 = int(self.params['size4'])
        filename34 = self.params['filename34']
        filename4 = self.params['filename4']
