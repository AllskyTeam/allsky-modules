...
def readAllsky():
	...
	environment_data = s.load_extra_data_file('allskytemp.json')
	temperature = s.get_allsky_variable('AS_TEMP')
	humidity = s.get_allsky_variable('AS_HUMIDITY')
	dew_point = s.get_allsky_variable('AS_DEW')
	heat_index = s.get_allsky_variable('AS_HEATINDEX')
	pressure = s.get_allsky_variable('AS_PRESSURE')
	rel_humidity = s.get_allsky_variable('AS_RELHUMIDITY')
	altitude = s.get_allsky_variable('AS_ALTITUDE')
...
def getSensorReading(sensorType, inputpin, i2caddress, dhtxxretrycount, dhtxxdelay, sht31heater, soloURL, params):
	...
	elif sensorType == 'Allsky':
		temperature, humidity, pressure, relHumidity, altitude = readAllsky()
	else:
		...
		if shouldRun or debugMode:                        
			...
			if heaterpin != 0:
				...
				if lastRunTime is not None:
					...
					if (lastRunSecs >= frequency) or debugMode:
