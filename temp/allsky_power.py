...
class ALLSKYPOWER(ALLSKYMODULEBASE):
	...
	def __ina219(self):
		...
		try:
			...
			result = f'__ina219 failed on line {eTraceback.tb_lineno} - {e}'
			allsky_shared.log(0, f'ERROR: {result}')
		
		return result, extra_data
    		
	def __read_ina3221_channel(self, ina3221, channel):
		...
		try:
			...
			for i in range(1, 4):
				...
				if channel_enabled:
					...
					voltage, current, bus_voltage, shunt_voltage, power = self.__read_ina3221_channel(ina3221, i)
			...
			eType, eObject, eTraceback = sys.exc_info()
			result = f'__ina3221 failed on line {eTraceback.tb_lineno} - {e}'
			allsky_shared.log(0, f'ERROR: {result}')
	...
	def run(self):
		...
		extra_data_filename = 'allsky_power.json'
		sensor_type = self.get_param('type', '', str)  
		if sensor_type == 'ina3221':
			result, extra_data = self.__ina3221()
		else:
			if sensor_type == 'ina219':
				result, extra_data = self.__ina219()       
			else:
				result = f'power module - invalid sensor type "{sensor_type}"'

		allsky_shared.save_extra_data(extra_data_filename, extra_data, metaData['module'], metaData['extradata'])
