...
class ALLSKYTEMP(ALLSKYMODULEBASE):
	...
	def __create_cardinal(self, degrees):
		try:
	...
	def __process_ow_result(self, data, expires, units):
		extra_data = {}
		...
		self.__set_extra_value('weather.main', data, 'OWWEATHER', expires, extra_data)
		self.__set_extra_value('weather.description', data, 'OWWEATHERDESCRIPTION', expires, extra_data)

		self.__set_extra_value('main.temp', data, 'OWTEMP', expires, extra_data)
		self.__set_extra_value('main.feels_like', data, 'OWTEMPFEELSLIKE', expires, extra_data)
		self.__set_extra_value('main.temp_min', data, 'OWTEMPMIN', expires, extra_data)
		self.__set_extra_value('main.temp_max', data, 'OWTEMPMAX', expires, extra_data)
		self.__set_extra_value('main.pressure', data, 'OWPRESSURE', expires, extra_data)
		self.__set_extra_value('main.humidity', data, 'OWHUMIDITY', expires, extra_data)

		self.__set_extra_value('wind.speed', data, 'OWWINDSPEED', expires, extra_data)
		self.__set_extra_value('wind.deg', data, 'OWWINDDIRECTION', expires, extra_data)
		self.__set_extra_value('wind.gust', data, 'OWWINDGUST', expires, extra_data)

		self.__set_extra_value('clouds.all', data, 'OWCLOUDS', expires, extra_data)

		self.__set_extra_value('rain.1hr', data, 'OWRAIN1HR', expires, extra_data)
		self.__set_extra_value('rain.3hr', data, 'OWRAIN3HR', expires, extra_data)

		self.__set_extra_value('sys.sunrise', data, 'OWSUNRISE', expires, extra_data)
		self.__set_extra_value('sys.sunset', data, 'OWSUNSET', expires, extra_data)

		temperature = float(self.__get_value('main.temp', data))
		humidity = float(self.__get_value('main.humidity', data))
		if units == 'imperial':
		...
		degress = self.__get_value('wind.deg', data)
		cardinal = self.__create_cardinal(degress)
	...
	def __set_extra_value(self, path, data, extra_key, expires, extra_data):
		value = self.__get_value(path, data)
	...
	def __get_ow_values(self, file_name):
		temperature = None
		humidity = None
		pressure = None
		the_dew_point = None
		...
		if os.path.isfile(extra_data_fileName):
			...
			with open(extra_data_fileName, 'r', encoding='utf-8') as file:
				...
				temperature = self.__get_ow_Value('AS_OWTEMP', json_data, file_modified_time)
				humidity = self.__get_ow_Value('AS_OWHUMIDITY', json_data, file_modified_time)
				pressure = self.__get_ow_Value('AS_OWPRESSURE', json_data, file_modified_time)
				the_dew_point = self.__get_ow_Value('AS_OWDEWPOINT', json_data, file_modified_time)

		return temperature, humidity, pressure, the_dew_point

	def __get_ow_Value(self, field, json_data, file_modified_time):
		result = False    
	...
	def __get_value(self, path, data):
		result = None
		keys = path.split('.')
		if keys[0] in data:
	...
	def __read_open_weather(self, sensor_number):
		...
		try:
			...
			if should_run or self.__debugmode:
				if api_key != '':
					if file_name != '':
							...
							if lon is not None and lon != '':
								...
								try:
									...
									if response.status_code == 200:
										...
										extra_data = self.__process_ow_result(raw_data, expire, units)
			...
			temperature, humidity, pressure, the_dew_point = self.__get_ow_values(file_name)                
		except Exception as e:
			eType, eObject, eTraceback = sys.exc_info()
			allsky_shared.log(0, f'ERROR: Module readOpenWeather failed on line {eTraceback.tb_lineno} - {e}')
	...
	def __read_ds18B20(self, sensor_number):
		humidity = None
		temperature = None
	...
	def __read_htu21(self, sensor_number):
		temperature = None
		humidity = None
	...
	def __read_ahtx0(self, sensor_number):
		temperature = None
		humidity = None
	...
	def __do_dhtxx_read(self, input_pin):
		temperature = None
		humidity = None
	...
	def __read_dht22(self, sensor_number):
		temperature = None
		humidity = None
		count = 0
		reading = True
		...
		while reading:
			temperature, humidity = self.__do_dhtxx_read(input_pin)
	...
	def __read_bme280_i2c(self, sensor_number):
		temperature = None
		humidity = None
		pressure = None
		relHumidity = None
	...
	def __read_sht31(self, sensor_number):
		temperature = None
		humidity = None
	...
	def __get_sensor_reading(self, sensor_type, sensor_number):
		temperature = None
		humidity = None
		the_dew_point = None
		pressure = None
		...
		if sensor_type == 'SHT31':
			temperature, humidity = self.__read_sht31(sensor_number)
		elif sensor_type == 'DHT22' or sensor_type == 'DHT11' or sensor_type == 'AM2302':
			temperature, humidity = self.__read_dht22(sensor_number)
		elif sensor_type == 'BME280-I2C':
			temperature, humidity, pressure, altitude = self.__read_bme280_i2c(sensor_number)
		elif sensor_type == 'HTU21':
			temperature, humidity = self.__read_htu21(sensor_number)
		elif sensor_type == 'AHTx0':
			temperature, humidity = self.__read_ahtx0(sensor_number)
		elif sensor_type == 'DS18B20':
			temperature, humidity = self.__read_ds18B20(sensor_number)
		elif sensor_type == 'OpenWeather':
			temperature, humidity, pressure, the_dew_point = self.__read_open_weather(sensor_number)        
		else:
	...
	def __debug_output(self, sensor_type, temperature, humidity, the_dew_point, pressure, rel_humidity, altitude):
	...
	def run(self):
		result = ''
		extra_data = {}
		extra_data_filename = 'allskytemp.json'
		self.__run_interval = self.get_param('frequency', 60, int)  
		self.__debugmode = self.get_param('ALLSKYTESTMODE', False, bool)    

		should_run, diff = allsky_shared.shouldRun('allskytemp', self.__run_interval)      
		...
		if should_run or self.__debugmode:
			...
			for sensor_number_itr in range(1,5):
				...
				if sensor_type != 'None':
					...
					temperature, humidity, the_dew_point, pressure, rel_humidity, altitude = self.__get_sensor_reading(sensor_type, sensor_number)
					if temperature is not None:
						temperature = round(temperature, 2)
					...
					self.__debug_output(sensor_type, temperature, humidity, the_dew_point, pressure, rel_humidity, altitude)
			...
			allsky_shared.saveExtraData(extra_data_filename, extra_data, metaData['module'], metaData['extradata'])

		else:
			result = 'Will run in {:.2f} seconds'.format(self.__run_interval - diff)
			allsky_shared.log(1, f'INFO: {result}')
