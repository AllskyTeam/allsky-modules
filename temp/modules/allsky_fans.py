...
class ALLSKYFANS:
	...
	__event = ''
	__temperature = 0
	__temperature_limit = 0
	__fan_pin = 0
	__invert_relay = False
	__debugmode = False
	__pwm_map = {
		"4B": {
	...
	def __get_cpu_temperature(self):
		...
		try:
			...
			result = f'Module __get_cpu_temperature - {exception_traceback.tb_lineno} - {e}'
			allsky_shared.log(4, f'ERROR: {result}')   
     
		return temp_c

	def __get_allsky_temperature(self):
		temperature = 0
		try:
			temperature = allsky_shared.get_allsky_variable(self.__params['temperature'])
	...
	def __get_param(self, param, default, target_type=str):
		result = default
		try:
			result = self.__params[param]
		except ValueError:
			pass
	...
	def __turn_fan_on(self, fan_pin, invert_relay):
	...
	def __turn_fan_off(self, fan_pin, invert_relay):
		pin = DigitalInOut(fan_pin)
		pin.switch_to_output()
	...
	def __display_status(self, value):
		return 'On' if value else 'Off'
    
	def __use_bool_fan_control(self):
		error = False
         
		if (self.__temperature > self.__temperature_limit):
			self.__turn_fan_on(self.__fan_pin, self.__invert_relay)
			fan_status = True
			result = f'{self.__temperature} is higher then set limit of {self.__temperature_limit}, Fans are {self.__display_status(fan_status)} via fan pin {self.__fan_pin}'
		else:
			self.__turn_fan_off(self.__fan_pin, self.__invert_relay)
			fan_status = False
			result = f'{self.__temperature} is lower then set limit of {self.__temperature_limit}, Fans are {self.__display_status(fan_status)} via fan pin {self.__fan_pin}'

		return result, error
     
	def __use_pwm_fan_control(self):
		pwm_min = self.__get_param('pwmmin', 0, int)
		pwm_max = self.__get_param('pwmmax', 0, int)
		result = ''
		pwm_enabled = '0'
		pwm_duty_cycle = 0
		error = False
		...
		try:
			if self.__fan_pin != 0:
				...
				model = pi_info.model
				if model in self.__pwm_map:
					pwm_channel =  self.__pwm_map[model]['addresses'][self.__fan_pin]
					enabled_file = self.__pwm_map[model]['enabled']
					chip = self.__pwm_map[model]['chip']
					...
					if self.__temperature <= pwm_min:
						pwm_duty_cycle = 0
					elif self.__temperature > pwm_max:
						pwm_duty_cycle = 100
					else:
						pwm_duty_cycle = int(((self.__temperature - pwm_min) / (pwm_max - pwm_min)) * 100)
					...
					result = f'PWM duty cycle set to {pwm_duty_cycle} on pin {self.__fan_pin}'
				else:
			...
			exception_type, exception_object, exception_traceback = sys.exc_info()
			result = f'Module __use_pwm_fan_control - {exception_traceback.tb_lineno} - {e}'
			allsky_shared.log(4, f'ERROR: {result}')      
	...
	def run(self):
		result = ''
		fan_status = ''
		sensor_type = self.__get_param('sensor_type', 'internal')
		run_period = self.__get_param('period', 60, int)
		self.__temperature_limit = self.__get_param('limitInternal', 0, int)
		self.__fan_pin = self.__get_param('fanpin', None, int)
		self.__invert_relay = self.__get_param('invertrelay', False, bool)
		self.__debugmode = self.__get_param('ALLSKYTESTMODE', False, bool)  
		use_pwm = self.__get_param('usepwm', False, bool)
		...
		try:
			...
			if should_run or self.__debugmode:
				extra_data = {}
				if self.__fan_pin is not None:

					if sensor_type == 'Internal':
						self.__temperature = self.__get_cpu_temperature()
					if sensor_type == 'Allsky':
						self.__temperature = self.__get_allsky_temperature()

					if self.__temperature is not None:
						if use_pwm:
							result, pwm_duty_cycle, pwm_enabled, error = self.__use_pwm_fan_control()
						else:
							self.__fan_pin = allsky_shared.getGPIOPin(self.__fan_pin)         
							result, error = self.__use_bool_fan_control()

						if not error:
							extra_data['AS_FANS_FAN_STATE'] = fan_status
							extra_data['AS_FANS_TEMP_LIMIT'] = self.__temperature_limit
							extra_data['AS_FANS_TEMPERATURE'] = self.__temperature
							if use_pwm:
							...
							allsky_shared.saveExtraData('allskyfans.json', extra_data, metaData['extradata'])
