...
class ALLSKYGPS(ALLSKYMODULEBASE):
	...
	def __compare_gps_and_allsky(self, lat, lon):
		...
		allsky_lat = float(self.__truncate(allsky_lat[:-1]))
		allsky_lon = float(self.__truncate(allsky_lon[:-1]))
		...
		lat = float(self.__truncate(lat[:-1]))
		lon = float(self.__truncate(lon[:-1]))
	...
	def __check_time_sync_running(self):
		active = False
	...
	def __deg_to_dms(self, deg, type='lat'):
	...
	def run(self):
		...
		extra_data_filename = 'pigps.json'
		...
		if should_run or self.__debugmode:
			if set_time:
				if not self.__check_time_sync_running():
					try:
						timeout = time.time() + 3
						while True:
							report = gpsd.next()
					...
					if report['class'] == 'TPV':
						...
						if mode != MODE_NO_FIX:
							if lat != 0 and lon != 0:
								...
								discResult, discAllSkyLat, discAllSkyLon, strLat, strLon = self.__compare_gps_and_allsky(lat, lon)
								if discResult:
								...
								extra_data['AS_PIGPSLATDEC'] = strLat
								extra_data['AS_PIGPSLONDEC'] = strLon
								extra_data['AS_PIGPSLAT'] = self.__deg_to_dms(lat, 'lat')
								extra_data['AS_PIGPSLON'] = self.__deg_to_dms(lon, 'lon')
