...
| Recipient(s)                      | The address(es) to receive the email notifications.  ||
| Subject                           | Text for email subject line ||
| Append date                       | Select to append Date Stamp to subject line text |Yes|
...
||||
 **Test**           |||
 |The [Test Module] button will trigger an email that sends the prior night's images. Use to verify server/credentials/recipient/email body text, etc.||
<hr>

### Notes:
 - For Gmail with MFA enabled you need an app password (not your regular login password). See [Sign in with app passwords](https://support.google.com/accounts/answer/185833?hl=en) for details on how to setup.  _eg a 16-digit passcode used to give the module permission to access your Google Account to send email._
